...
#define BETRAY_BUTTON_ALT /* Legacy alias added for compatibility */
...
/* Legacy alias mapping. If BETRAY_BUTTON_MENU is later turned into a value, ALT will follow. */
#ifdef BETRAY_BUTTON_MENU
#undef BETRAY_BUTTON_ALT
#define BETRAY_BUTTON_ALT BETRAY_BUTTON_MENU
#endif
...
// Legacy safe-area API (removed in newer versions) -- compatibility shim
// Some legacy modules (e.g., s_sort.c) expect safe-area query via betray_screen_mode_safe_get
// with enum-like constants BETRAY_SA_ACTION and BETRAY_SA_TITLE representing two UI bands.
// We expose minimal placeholders mapping to full screen for both. Adjust if real safe-areas needed.
#ifndef BETRAY_SCREEN_MODE_SAFE_COMPAT
#define BETRAY_SCREEN_MODE_SAFE_COMPAT
#define BETRAY_SA_ACTION 0
#define BETRAY_SA_TITLE 1
static inline void betray_screen_mode_safe_get(uint area, float *left, float *right, float *top, float *bottom)
{
    uint _sx, _sy;
    boolean _fs;
    double aspect = betray_screen_mode_get(&_sx, &_sy, &_fs);
    (void)aspect; /* aspect unused but kept for possible future calc */
    if (left)
        *left = -1.0f; /* normalized coordinates matching rest of UI expectations */
    if (right)
        *right = 1.0f;
    if (top)
        *top = 1.0f;
    if (bottom)
        *bottom = -1.0f;
    (void)area; /* both areas map to full screen in absence of specific data */
}
#endif
...
extern void betray_view_direction(float *matrix); /* A 4X4 rotation matrix  that should be aplied to all drawing in order to support multi display solutions and headmounted disp�lays. */
...
extern uint betray_image_device_count();                                          /* Not yet implemented. */
extern char *betray_image_device_name(uint device_id);                            /* Not yet implemented. */
extern void betray_image_device_size(uint device_id, uint *size_x, uint *size_z); /* Not yet implemented. */
extern void betray_image_device_content(uint device_id);                          /* Not yet implemented. */
extern void betray_image_device_type(uint device_id);                             /* Not yet implemented. */
extern void betray_image_device_data_get(uint device_id, void *buffer);           /* Not yet implemented. */
extern void betray_image_device_data_set(uint device_id, void *buffer);           /* Not yet implemented. */

extern void betray_image_device_capture_begin(uint device_id);           /* Not yet implemented. */
extern void betray_image_device_capture_end(uint device_id);             /* Not yet implemented. */
extern uint betray_image_device_capture_frame_available(uint device_id); /* Not yet implemented. */

extern boolean betray_image_device_output_available(uint device_id); /* Not yet implemented. */
...
typedef enum
{
    ...
    BETRAY_ST_2D,           /* A two dimensional value ranged form, minus one to plus one. */
...
extern boolean betray_settings_trigger(uint id); /* Triggers a setting with the type BETRAY_ST_TOGGLE. */
