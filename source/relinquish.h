/* Include guard to prevent multiple inclusion re-definitions */
#ifndef RELINQUISH_H
#define RELINQUISH_H
...
{
#endif
/* Depend on forge typedefs (boolean, uint, uint8, uint64, etc.) */
#include "forge.h"
    /*  -------------- Reliquish --------------------
    ...
    */
    ...
    /* --- Backwards compatibility layer ---
       Some legacy Seduce/Relinquish source files (fonts, widgets, particles, etc.)
       still refer to the old vertex format enum name "RFormats" and the token
       "R_FLOAT". The current API exposes these as SUIFormats / SUI_FLOAT. To keep
       changes minimal and avoid touching many source files, we provide a simple
       compatibility typedef and macro mapping. If more old tokens show up later
       (e.g. R_UINT16) they can be mapped here the same way, but so far only
       R_FLOAT is referenced in the project sources. */
    typedef SUIFormats RFormats; /* old name -> new name */
#ifndef R_FLOAT
#define R_FLOAT SUI_FLOAT
#endif
...
/* --- Legacy Compatibility Section ---
    Older code paths expect some symbols that no longer exist or whose usage has changed.
    We provide light‑weight shims here so we don't have to touch multiple source files.
*/
#ifndef RELINQUISH_LEGACY_COMPAT
#define RELINQUISH_LEGACY_COMPAT
/* Legacy framebuffer texture target enum value (was passed into r_framebuffer_allocate in older API).
    Current signature omits the target parameter; calls still pass RELINQUISH_TARGET_2D_TEXTURE. Safe to define dummy. */
#ifndef RELINQUISH_TARGET_2D_TEXTURE
#define RELINQUISH_TARGET_2D_TEXTURE 0
#endif

/* Legacy extension query enums. Only R_Q_GEOMETRY_OUTPUT_COMPONENTS_MAX used (s_lines.c) to size an internal buffer.
    Provide a conservative upper bound typical for modern desktop GL (e.g., max geometry output components often 128 or 256).
    Original code divides the queried value by 24; using 96*4 (=384) ensures sufficient size without being huge.
*/
#ifndef R_Q_GEOMETRY_OUTPUT_COMPONENTS_MAX
#define R_Q_GEOMETRY_OUTPUT_COMPONENTS_MAX 384
#endif

/* Provide a minimal stub for r_extension_query if not present. If an actual implementation exists elsewhere, this stays unused. */
#ifndef RELINQUISH_EXTENSION_QUERY_DECLARED
    static inline uint r_extension_query(uint query)
    {
        /* Only legacy use case expects R_Q_GEOMETRY_OUTPUT_COMPONENTS_MAX */
        if (query == R_Q_GEOMETRY_OUTPUT_COMPONENTS_MAX)
            return R_Q_GEOMETRY_OUTPUT_COMPONENTS_MAX;
        return 0;
    }
#endif
#endif /* RELINQUISH_LEGACY_COMPAT */
