...
void seduce_primitive_line_init()
{
    ...
    if (seduce_global_line_shader == NULL)
    {
        printf("WARNING: Expand shader failed. Attempting simplified fallback.\n");
        /* Simplified fallback inline sources */
        {
            char *v = "uniform mat4 ModelViewMatrix;\nattribute vec4 vertex;\nvoid main(){gl_Position = ModelViewMatrix * vec4(vertex.xyz,1.0);}";
            char *g = "layout(lines) in;layout(triangle_strip,max_vertices=8) out;uniform mat4 ProjectionMatrix;uniform float distance;uniform float test;uniform float resolution;out vec4 uv;void main(){vec3 vector=normalize(gl_in[0].gl_Position.xyz-gl_in[1].gl_Position.xyz);float da=0.4*(gl_in[0].gl_Position.z/distance+1.0);float db=0.4*(gl_in[1].gl_Position.z/distance+1.0);float scale=max(abs(da)+0.05,abs(db)+0.05);vec3 expand=normalize(cross(normalize(gl_in[0].gl_Position.xyz),vector))*vec3(resolution*distance)*vec3(scale);float dist=0.5+0.5*length(gl_in[0].gl_Position.xyz-gl_in[1].gl_Position.xyz)/(resolution*scale);vec3 side=vector*vec3(0.02)*vec3(scale);uv=vec4(scale,scale,0,dist);gl_Position=ProjectionMatrix*(gl_in[0].gl_Position+vec4(expand+side,0.0));EmitVertex();uv=vec4(scale,-scale,0,dist);gl_Position=ProjectionMatrix*(gl_in[0].gl_Position-vec4(expand-side,0.0));EmitVertex();uv=vec4(0,scale,0.5,dist);gl_Position=ProjectionMatrix*(gl_in[0].gl_Position+vec4(expand,0.0));EmitVertex();uv=vec4(0,-scale,0.5,dist);gl_Position=ProjectionMatrix*(gl_in[0].gl_Position-vec4(expand,0.0));EmitVertex();uv=vec4(0,scale,dist,0.5);gl_Position=ProjectionMatrix*(gl_in[1].gl_Position+vec4(expand,0.0));EmitVertex();uv=vec4(0,-scale,dist,0.5);gl_Position=ProjectionMatrix*(gl_in[1].gl_Position-vec4(expand,0.0));EmitVertex();uv=vec4(-scale,scale,dist,0);gl_Position=ProjectionMatrix*(gl_in[1].gl_Position+vec4(expand-side,0.0));EmitVertex();uv=vec4(-scale,-scale,dist,0);gl_Position=ProjectionMatrix*(gl_in[1].gl_Position-vec4(expand+side,0.0));EmitVertex();EndPrimitive();}";
            char *f = "uniform float time; in vec4 uv; void main(){ gl_FragColor = vec4(1,0,0,1);}";
            char *fb_src[3] = {v, g, f};
            uint fb_stages[3] = {GL_VERTEX_SHADER_ARB, GL_GEOMETRY_SHADER_ARB, GL_FRAGMENT_SHADER_ARB};
            seduce_global_line_shader = r_shader_create(NULL, 0, fb_src, fb_stages, 3, "Expand shader fallback", NULL);
            if (seduce_global_line_shader == NULL)
            {
                printf("WARNING: Fallback expand shader also failed. Disabling line rendering.\n");
                return;
            }
            else
            {
                printf("INFO: Using fallback expand shader.\n");
            }
        }
    }
