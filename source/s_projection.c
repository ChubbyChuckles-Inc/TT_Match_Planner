...
{
    /* Legacy compatibility: some builds lacked BETRAY_BUTTON_ALT mapping; define fallback to MENU if missing */
#ifndef BETRAY_BUTTON_ALT
#define BETRAY_BUTTON_ALT BETRAY_BUTTON_MENU
    ...
    if (input->mode == BAM_EVENT)
    {
        ...
        for (i = 0; i < input->pointer_count; i++)
        {
            ...
            {
                if (id == seduce_element_pointer_id(input, i, NULL) && ((input->pointers[i].button[0] && !input->pointers[i].last_button[0]) ||
