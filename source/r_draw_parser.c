...
void r_shader_parse_clean(char *shader, char *output, RShaderInput *input, uint input_count, uint instance_id, boolean fragment_add, boolean pass_to_fragment)
{
    ...
    if (pass_to_fragment)
    {
        /* Avoid injecting relinquish_instance_id for geometry shaders to prevent use of gl_InstanceID/gl_BaseInstanceARB when not universally supported */
        if (!strstr(shader, "layout(triangle_strip") && !strstr(shader, "layout(lines)"))
        {
    ...
    for (i = 0; shader[i] != 0;)
    {
        ...
        if (out == 0 && pass_to_fragment)
        {
            ...
            if (main[k] == 0)
            {
                ...
                {
                    output[j++] = shader[i++];
                    /* Only inject instance id logic in non-geometry shaders */
                    if (!strstr(shader, "layout(triangle_strip") && !strstr(shader, "layout(lines)"))
                        for (l = 0; move[l] != 0; l++)
                            output[j++] = move[l];
                }
...
uint shader_parse_input_print(char *output, uint output_size, RShaderInput *input, uint input_count, RShaderInputMode mode, uint index_block, uint stage)
{
            ...
            else
            {
                ...
                if (input[i].stages[stage])
                {
                    ...
                    else
                    {
                        ...
                        {
                            char uniform_footer[256];
                            for (i = 0; i < input_count; i++)
                                if (input[i].qualifyer == R_SIQ_BLOCK && input[i].block == k)
                                    break;
                            /* No layout(binding), binding is set via glUniformBlockBinding in C */
    ...
    for (k = R_SIQ_ATTRIBUTE; k < R_SIQ_BLOCK; k++)
    {
        for (i = 0; i < input_count; i++)
        {
            if (input[i].qualifyer == k && input[i].stages[stage])
            {
                ...
                {
                    if (stage == 0 || input[i].stages[stage + 1])
                        q = "out";
                    else
                        q = "in";
                }
...
uint r_parse_shaders(char **output, char **shader, uint *stages, uint shader_count, RShaderInput *input, RShaderInputMode mode, uint version, boolean embeded, char *instance_name, uint *instance_id)
{
    ...
    for (i = 0; i < shader_count; i++)
    {
        ...
        r_shader_parse(input, &count, shader[i], "varying", 0, R_SIT_VDOUBLE4 + 1, R_SIQ_INOUT, i);
        r_shader_parse(input, &count, shader[i], "in", 0, R_SIT_VDOUBLE4 + 1, R_SIQ_INOUT, i);
        r_shader_parse(input, &count, shader[i], "out", 0, R_SIT_VDOUBLE4 + 1, R_SIQ_INOUT, i);
    }
    ...
    *instance_id = shader_parse_instance_id_get(input, count, mode, instance_name);
    ...
    for (i = 0; i < shader_count; i++)
    {
        /* In FLAT mode, keep original shader source (no UBO/block rewrite) */
        if (mode == R_SIM_FLAT)
        {
            uint sLen;
            for (sLen = 0; shader[i][sLen] != 0; sLen++)
                ;
            output[i] = malloc(sLen + 64);
            pos = 0;
            if (version != 0)
                pos += sprintf(&output[i][pos], "#version %u\n", version);
            for (j = 0; j < sLen; j++)
                output[i][pos++] = shader[i][j];
            output[i][pos] = 0;
        }
        else
        {
            ...
            if (version != 0)
            {
                pos += sprintf(&output[i][pos], "#version %u\n", version);
                if (r_glGetTextureHandleARB != NULL)
                    pos += sprintf(&output[i][pos], "#extension GL_ARB_bindless_texture : enable\n");
                /* Needed for gl_BaseInstanceARB usage inserted by the parser */
                pos += sprintf(&output[i][pos], "#extension GL_ARB_shader_draw_parameters : enable\n");
            }
            pos += shader_parse_input_print(&output[i][pos], 1024 * 1024, input, count, mode, *instance_id, i);
