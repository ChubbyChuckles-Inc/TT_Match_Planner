...
void r_shader_state_set_default(RState *state)
{
    ...
    state->cull_face = 0;
    state->blend_dest = GL_ONE;
    state->blend_source = GL_ONE_MINUS_SRC_ALPHA;
...
void r_shader_state_set_blend_mode(RShader *shader, uint blend_source, uint blend_destination)
{
    ...
    shader->state.blend_dest = blend_source;
    shader->state.blend_source = blend_destination;
}
...
boolean r_shader_init(void)
{
    ...
    if (r_extension_test("GL_ARB_shading_language_100") || TRUE)
    {
        ...
        /* Use core function name for program query (no ARB suffix) */
        r_glGetProgramivARB = r_extension_get_address("glGetProgramiv");
        ...
        /* Prefer core info log functions, fallback to ARB alias if needed */
        ...
        if (r_glGetShaderInfoLog == NULL || r_glGetProgramInfoLog == NULL)
        {
            GLvoid(APIENTRY * getInfoARB)(GLhandleARB object, GLsizei maxLenght, GLsizei * length, char *infoLog);
            getInfoARB = r_extension_get_address("glGetInfoLogARB");
            r_glGetShaderInfoLog = (void *)getInfoARB;
            r_glGetProgramInfoLog = (void *)getInfoARB;
        }
...
void r_shader_debug_print_uniform_buffer(RShader *shader, uint8 *uniform_buffer, char *text, uint buffer_id)
{
    ...
    else
    {
        ...
        for (i = 0; i < block_size; i += 4)
        {
            ...
            printf("r_shader_init: function pointers loaded\n");
        }

        printf("r_shader_init: GLSL 1.00 NOT present\n");
    }
}
...
void *r_shader_create(char *debug_output, uint output_size, char **shaders, uint *stages, uint stage_count, char *name, char *instance_block)
{
    ...
    shader->name[i] = 0;
    /* Lower GLSL version from 400 to 330 to improve compatibility on legacy profiles and avoid driver complaining about gl_InstanceID/gl_BaseInstanceARB in geometry stage. */
    input_count = r_parse_shaders(output, shaders, stages, stage_count, input, shader->mode, 330, FALSE, instance_block, &shader->instance_block);
    ...
    {
        text[0] = output[i];
        printf("r_shader_create: compiling stage %s for %s\n", stages[i] == GL_VERTEX_SHADER_ARB ? "VERTEX" : (stages[i] == GL_FRAGMENT_SHADER_ARB ? "FRAGMENT" : "OTHER"), name);
        ...
        if (!status)
        {
            ...
            /* Print compile log for easier diagnosis */
            printf("%s\n", debug_output);
            /* Also print start of shader source for context */
            {
                uint k;
                printf("Shader source head (%s):\n", stages[i] == GL_VERTEX_SHADER_ARB ? "VERTEX" : "FRAGMENT");
                for (k = 0; output[i][k] && k < 512; k++)
                    putchar(output[i][k]);
                putchar('\n');
            }
            free(debug_output);
    ...
    if (r_glTransformFeedbackVaryings != NULL)
    {
        char *names[64];
        uint output_count = 0;
        /* Find the stage immediately preceding the fragment stage (if any) */
        for (i = 1; i < stage_count && stages[i] != GL_FRAGMENT_SHADER_ARB; i++)
            ;
        if (i > 0)
            i--; /* i now indexes the stage before fragment (e.g., vertex in VS+FS pipeline) */
        for (j = 0; j < input_count; j++)
        {
            /* Capture varyings that are present in both the stage before fragment (i)
               and the fragment stage itself (i+1). Guard against overflow. */
            if (input[j].qualifyer == R_SIQ_INOUT && input[j].stages[i] && (i + 1 < R_MAX_SHADER_STAGES) && input[j].stages[i + 1])
            {
        ...
        if (input[i].qualifyer == R_SIQ_BLOCK)
        {
            ...
            if (shader->mode != R_SIM_FLAT)
            {
                if (shader->instance_block == shader->block_count)
                    ...
                    j = bind_point++;
                {
                    GLuint block_index = r_glGetUniformBlockIndex(shader->program, shader->blocks[shader->block_count].name);
                    if (block_index == 0xFFFFFFFFu)
                    {
                        printf("r_shader_create: WARNING uniform block '%s' not found in program %u for shader '%s'; fallback to FLAT uniforms.\n", shader->blocks[shader->block_count].name, shader->program, shader->name);
                        shader->mode = R_SIM_FLAT; /* disable further UBO usage for this shader */
                    }
                    else
                    {
                        GLint max_bindings = 0;
                        glGetIntegerv(0x8A2F /*GL_MAX_UNIFORM_BUFFER_BINDINGS*/, &max_bindings);
                        if (j >= (uint)max_bindings)
                        {
                            printf("r_shader_create: WARNING binding index %u exceeds GL_MAX_UNIFORM_BUFFER_BINDINGS (%d) for shader '%s'; forcing 0.\n", j, max_bindings, shader->name);
                            j = 0;
                        }
                        r_glUniformBlockBinding(shader->program, block_index, j);
                        shader->blocks[shader->block_count].id = j;
                    }
                }
                if (shader->mode != R_SIM_FLAT && (shader->blocks[shader->block_count].qualifyer == R_SIQ_BLOCK || r_glGetTextureHandleARB != NULL))
                {
...
void *r_shader_create_simple(char *debug_output, uint output_size, char *vertex, char *fragment, char *name)
{
    char *shaders[2];
    uint stages[2];
    printf("r_shader_create_simple: %s begin\n", name ? name : "<unnamed>");
    stages[0] = GL_VERTEX_SHADER_ARB;
    stages[1] = GL_FRAGMENT_SHADER_ARB;
    shaders[0] = vertex;
    shaders[1] = fragment;
    RShader *s = r_shader_create(debug_output, output_size, shaders, stages, 2, name, NULL);
    printf("r_shader_create_simple: %s %s\n", name ? name : "<unnamed>", s ? "ok" : "FAILED");
    return s;
}
