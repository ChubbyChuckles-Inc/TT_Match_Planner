...
Added for legacy Seduce text system. Implemented in f_utf8.c.
f_utf8_to_uint32 : decode code point at text[*pos], advance *pos, return code point (0xFFFD on malformed sequence)
f_uint32_to_utf8 : encode code point to out, return bytes written (0 if invalid)
*/
extern uint32 f_utf8_to_uint32(const char *text, uint *pos);
extern uint f_uint32_to_utf8(uint32 codepoint, char *out);

/* ---- Sorting ------
A fast Merge sort implementations for sorting integers and pointers */

extern uint *f_sort_ids(uint length, uint *ids, boolean (*compate_func)(uint big, uint smaller, void *user), void *user);
extern void **f_sort_pointers(uint length, void **pointers, boolean (*compate_func)(void *big, void *smaller, void *user), void *user);
