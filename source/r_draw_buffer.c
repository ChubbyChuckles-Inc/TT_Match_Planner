...
/* Attempt to grow an array pool so an upcoming allocation of "size_needed" vertices fits.
 * Returns TRUE on success (pool resized), FALSE on failure. */
static boolean r_array_grow(SUIArrayPool *p, uint size_needed)
{
    uint new_length;
    uint required = p->array_used + size_needed;
    /* Heuristic: double until it fits, with a minimum slack of 25%. */
    if (required <= p->array_length)
        return TRUE; /* already fits */
    new_length = p->array_length ? p->array_length : 1;
    while (new_length < required)
    {
        uint prev = new_length;
        new_length = new_length * 2;
        if (new_length < prev) /* overflow safeguard */
            break;
    }
    if (new_length < required)
        new_length = required; /* fallback if overflow logic triggered */

    /* Limit runaway allocations: cap at ~64M vertices (arbitrary large guard). */
    if (new_length > 64u * 1024u * 1024u)
    {
        fprintf(stderr, "r_array_grow: requested size too large (%u)\n", new_length);
        return FALSE;
    }
    ...
    {
        /* GPU buffer path */
        uint old_used_bytes = p->array_used * p->vertex_size;
        uint old_size_bytes = p->array_length * p->vertex_size;
        void *temp = NULL;
        if (old_used_bytes)
        {
            temp = malloc(old_used_bytes);
            if (temp == NULL)
            {
                fprintf(stderr, "r_array_grow: malloc failed (%u bytes)\n", old_used_bytes);
                return FALSE;
            }
            r_glBindBufferARB(GL_ARRAY_BUFFER_ARB, p->gl_buffer);
            r_glGetBufferSubDataARB(GL_ARRAY_BUFFER_ARB, 0, old_used_bytes, temp);
        }
        /* Reallocate buffer */
        r_glBindBufferARB(GL_ARRAY_BUFFER_ARB, p->gl_buffer);
        r_glBufferDataARB(GL_ARRAY_BUFFER_ARB, new_length * p->vertex_size, NULL, GL_DYNAMIC_DRAW_ARB);
        if (old_used_bytes && temp != NULL)
        {
            r_glBufferSubDataARB(GL_ARRAY_BUFFER_ARB, 0, old_used_bytes, temp);
            free(temp);
        }
        r_glBindBufferARB(GL_ARRAY_BUFFER_ARB, r_array_bound);
    }
    else
    {
        /* CPU memory path */
        uint8 *new_data;
        uint old_bytes = p->array_length * p->vertex_size;
        uint new_bytes = new_length * p->vertex_size;
        ...
        if (p->data && old_bytes)
            memcpy(new_data, p->data, old_bytes);
        if (p->data)
            free(p->data);
        p->data = new_data;
    }
    p->array_length = new_length;
    p->defragged = FALSE; /* growth might introduce fragmentation opportunities */
    fprintf(stderr, "r_array_grow: grew pool to %u vertices (vertex_size=%u)\n", p->array_length, p->vertex_size);
    return TRUE;
}
...
void *r_array_section_allocate_vertex(void *pool, uint size)
{
    ...
    if (p->array_used + size > p->array_length)
    {
        /* Try to grow instead of exiting. */
        if (!r_array_grow(p, size))
        {
            printf("r_array_section_allocate_vertex: out of memory %u %u %u\n", p->array_used, size, p->array_length);
            return NULL; /* Fail gracefully; caller should handle NULL. */
        }
    }
...
void r_array_init(void)
{
    ...
    printf("r_array_init: VBO ext = %d\n", r_buffer_extension);
    if (r_buffer_extension)
    {
        printf("r_array_init: loading VBO functions\n");
        ...
        {
            printf("r_array_init: VAO ext present, loading\n");
    ...
    printf("r_array_init: loaded misc functions\n");
#endif
    r_array_bound = 0;
    for (i = 0; i < 64; i++)
        r_array_attrib_mode[i] = FALSE;
    printf("r_array_init: end\n");
}
...
/* Correct prototype already declared later; removing stale incorrect extern */
